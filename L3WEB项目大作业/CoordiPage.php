...
<body>
<?php
  echo '<button onclick="addWeek()">Add Week</button>';
?>
...
<div class="container-a">
    <div class="container-bcd">
        ...
        <div class="container-c">         <h1 class="center" id="title">标题!!!!
                            </h1>               </div>
...
<div class="container-a" >
    <div class="container-bcd">
        ...
        <div class="container-d">              <?php
            echo '<button onclick="addWeek()">Add Week</button> ';
            ?>                   </div>
        <div class="container-d">              <?php
            echo '<input type="text" name="major" id="newmajortext">';
            ?>                   </div>

        <div class="container-d">              <?php
            echo '<button id="addMajorBtn " onclick=" addmajor()">AddMajor</button>';
            ?>                   </div>
...
<script>
        ...
        for (var i = 0; i <= 45; i++) { // i=45 代表我们课表的时间持续时间
            ...
            if (i == 0) {
                ...
                for (var j = 1; j <= 21; j++) { //这个循环代表每一行的每一列中要绘制什么
                    ...
                    var tdObj = document.getElementById(tdId); // 获取单元格对象
                    ...
                    var tdObj = document.getElementById(tdId); // 获取单元格对象
                    var btn = document.createElement('button'); // 创建一个按钮元素
                    var rowIndex = tdObj.parentNode.rowIndex; // 获取单元格所在行数
                    var colIndex = j;

                    btn.addEventListener('click', (function(rowIndex, colIndex) {
                        return function() {
                            // 按钮的点击事件处理程序
                            alert('line' + rowIndex + 'col' + colIndex);


                                // 点击按钮时，弹出迷你悬浮弹窗
                                var popup = window.open("", "popup", "width=800 ,height=50");
                                ...
                                inputTimeSE.value=rowIndex-1;
                                inputTimeSE.name=this.parentNode.getAttribute("tstart");
                                inputTimeSE.disabled = true;
                                ...
                                inputDate.value = Math.floor(colIndex / 3) + 1;
                                inputDate.name=this.parentNode.getAttribute("tend");
                                inputDate.disabled = true;


                                 var inputGroup = popup.document.createElement("input");
                                 inputGroup.value = (colIndex % 3) ;
                                 inputGroup.name=this.parentNode.getAttribute("group");
                                ...
                                form.appendChild(popup.document.createTextNode("group"));
                                ...
                                submitBtn.onclick = function() {
                                    var timeSEValue = inputTimeSE.value;
                                    var timeEndValue = inputTimeEnd.value;
                                    var dateValue = inputDate.value;
                                    var groupValue = inputGroup.value;

                                   // document.write(timeSEValue+timeEndValue+dateValue+groupValue);


                                    $.ajax({
                                        type: 'POST',
                                        url: 'CoordiAddCours.php',
                                        data: {
                                            "major":major,
                                            "week":weekinfoStr,
                                            "timeSE": timeSEValue,
                                        ...
                                        success: function(response) {
                                            alert(response);
                                            window.location.replace("CoordiPage.php");
                                        },
                    ...
                    tdObj.appendChild(btn); // 将按钮添加到单元格中
    ...
    function addmajor(){
        ...
        var newmajor=document.getElementById("newmajortext").value;
